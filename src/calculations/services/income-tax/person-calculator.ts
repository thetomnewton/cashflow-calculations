...
export class PersonIncomeTaxCalculator {
  constructor(
  ...
  calculate() {
    const incomes = this.incomesForPerson();
    ...
    this.taperAllowances(adjustedNetIncome);
    this.deductAllowances(incomes);
    this.extendTaxBands(incomes);
    this.useTaxBands(incomes);
  ...
  private incomesForPerson() {
    return this.cashflow.incomes.filter(({ people }) =>
      people.some(({ id }) => id === this.person.id)
    );
  }
  ...
  private taperAllowances(adjustedNetIncome: number) {
    this.taperPersonalAllowance(adjustedNetIncome);
   ...
   */
  private taperPersonalAllowance(adjustedNetIncome: number) {
    const pa = this.output.tax.bands[this.year.tax_year][this.person.id].find(
   ...
   */
  private deductAllowances(incomes: Income[]) {
   ...
   */
  private extendTaxBands(incomes: Income[]) {
   ...
   */
  private useTaxBands(incomes: Income[]) {
